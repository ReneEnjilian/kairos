...
INPUT_FILE = Path("end_to_end_periodic.jsonl")
OUTPUT_FILE = Path("end_to_end_periodic_latency.pdf")

SKIP_FIRST_N = 22

LATENCY_CORRECTION_START_INDEX = 1130   # original 1-based request index
LATENCY_CORRECTION_MS = 15.0

MODEL_SWITCH_INDEX = 1130               # original request index
...
with INPUT_FILE.open("r", encoding="utf-8") as f:
    for i, line in enumerate(f, start=1):
        ...
        # Apply correction from original request index 1130 onward.
        if i >= LATENCY_CORRECTION_START_INDEX:
            latency -= LATENCY_CORRECTION_MS

        # Avoid invalid values on log scale.
...
# Explicit ticks make the lower latency region easier to read.
